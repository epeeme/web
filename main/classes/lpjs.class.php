...
class lpjs extends DB {
    ...
    public function getSeriesResults($data) {
        ...
        foreach ($fencerData as $row) {
           ...
           for ($i=0; $i<count($seriesData); $i++) { 
                $lpjsData = $this->getEventPoints($seriesData[$i], $row['fencerID'], $data['catID']);
                $row['e'.$i.'_points'] = $lpjsData['points'];
                $row['e'.$i.'_place'] = $lpjsData['position'];
                $row['e'.$i.'_placeSuffix'] = $this->placeSuffix($lpjsData['position']);
    ...
    private function getEventPoints($data, $fencer, $cat) {
        ...
        $sql = $this->db->prepare('SELECT lpjsPoints AS points, results.eventPosition AS position 
                                   FROM results 
