(function(eliteEpee, $, undefined) {
    
    eliteEpee.Rankings = function() {
        ...
        var tableTitle = '';
        switch ($('.demographics form select[name="cat"]').val()) {
            case 'senior' : { 
                if (($('.demographics form select[name="sex"]').val()) == 'female') {
                    tableTitle = tableTitle + 'Senior Women\'s Elite Epee Series ';
                    $('input[name="series"]').val('EEWS');
                } else {
                    tableTitle = tableTitle + 'Senior Men\'s Elite Epee Series ';
                    $('input[name="series"]').val('EEMS');
                }
                break;
            }
            default : { 
                tableTitle = tableTitle + 'Elite Epee Junior Series | ' + $('.demographics form select[name="sex"]').val() + ' | ' + $('.demographics form select[name="cat"]').val();
                $('input[name="series"]').val('EEJS');
                break;
            }
        }

        $.ajax({
            url: 'main/c.php?m=eliteEpee&id=getSeriesSize&' + $('.demographics form').serialize(),
        }).done(function(cols) {
            ...
            var builtColumns = [];
            $.ajax({
                url: 'main/c.php?m=eliteEpee&id=getSeriesResults&' + $('.demographics form').serialize(),
               ...
               $('.index2 table').DataTable( {
                    dom: 't',
                    ajax: 'main/c.php?m=eliteEpee&id=getSeriesCompetitions&' + $('.demographics form').serialize(),
                    columnDefs: [ {
                        className: 'control',
                        orderable: false,
                        targets: -1
...
}(window.eliteEpee = window.eliteEpee || {}, jQuery));
