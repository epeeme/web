(function(lpjs, $, undefined) {
    
  lpjs.Rankings = function() {
      ...
      var tableTitle = 'LPJS | ' + $('.demographics form select[name="sex"]').val() + ' | ' + $('.demographics form select[name="cat"]').val();
      $('input[name="series"]').val('LPJS');

      $.ajax({
          url: 'main/c.php?m=lpjs&id=getSeriesSize&' + $('.demographics form').serialize(),
      }).done(function(cols) {
          ...
          var builtColumns = [];
          $.ajax({
              url: 'main/c.php?m=lpjs&id=getSeriesResults&' + $('.demographics form').serialize(),
             ...
             $('.index2 table').DataTable( {
                  dom: 't',
                  ajax: 'main/c.php?m=lpjs&id=getSeriesCompetitions&' + $('.demographics form').serialize(),
                  columnDefs: [ {
                      className: 'control',
                      orderable: false,
                      targets: -1
  ...
  function getCatID(cat, sex) {
      ...
      if (sex == 'male') {
          switch (cat) {
              case 'u9' : { catID = 10; break; }
              case 'u11' : { catID = 8; break; }
              case 'u13' : { catID = 3; break; }
              case 'u15' : { catID = 5; break; }
          }
      } else {
          switch (cat) {
              case 'u9' : { catID = 9; break; }
              case 'u11' : { catID = 7; break; }
...
}(window.lpjs = window.lpjs || {}, jQuery));
